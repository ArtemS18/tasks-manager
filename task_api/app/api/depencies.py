...
def login_service(req: Request, auth=Depends(jwt_service)):
    return LoginService(repository=req.app.store.user, jwt=auth, redis=req.app.store.redis)
...
async def validation_confirm_token(confirm_token: Annotated[ str | None, Cookie()], service:Annotated[LoginService, Depends(login_service)]):
    if confirm_token is None:
        raise  HTTPException( status_code=status.HTTP_403_FORBIDDEN, detail="Access token required. Please provide the 'confirm_token' cookie.")
