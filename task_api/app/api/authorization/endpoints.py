...
async def reg_user(
    ...
    return OKResponseSchema(
        message="Please confirm yout email (login)", details=user_response.model_dump()
    )
...
async def confirm_email(
    ...
    user: User = Depends(validation_confirm_token),
):
    await service.confirm_email(user, data.password)
    user_response = UserSchemaResponse.model_validate(user)
    response.delete_cookie("confirm_token")
    return OKResponseSchema(
