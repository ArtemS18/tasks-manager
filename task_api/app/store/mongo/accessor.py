...
class MongoAccessor(BaseAccessor):
    ...
    async def connect(self):
        self.client = AsyncIOMotorClient(self.app.config.mongo.url)
    ...
    async def create_message(self, ms: Message) -> int:
        new_ms = await self.db.messages.insert_one(
            ms.model_dump(by_alias=True, exclude_none=False, exclude={"id"})
        )
